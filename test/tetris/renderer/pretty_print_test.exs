defmodule Tetris.Renderer.PrettyPrintTest do
  use ExUnit.Case

  import ExUnit.CaptureIO

  alias Tetris.Renderer.PrettyPrint
  ...
  describe "render/1" do
    test "outputs score and rows cleared" do
      output = capture_io(fn -> PrettyPrint.render(render_data()) end)
      assert output =~ "Score: 10"
      assert output =~ "Rows: 2"
    end

    test "outputs LIVE status when game is live" do
      output = capture_io(fn -> PrettyPrint.render(render_data(%{status: :live})) end)
      assert output =~ "LIVE"
    end

    test "outputs PAUSED status when game is paused" do
      output = capture_io(fn -> PrettyPrint.render(render_data(%{status: :paused})) end)
      assert output =~ "PAUSED"
    end

    test "outputs GAME OVER status when game is over" do
      output = capture_io(fn -> PrettyPrint.render(render_data(%{status: :over})) end)
    ...
    test "renders captured cells with █" do
      ...
      output = capture_io(fn -> PrettyPrint.render(data) end)
      assert output =~ "█"
    end
    ...
    test "returns :ok" do
      result = capture_io(fn -> send(self(), PrettyPrint.render(render_data())) end)
      assert result =~ "Score"
      assert_received :ok
    end
  end
